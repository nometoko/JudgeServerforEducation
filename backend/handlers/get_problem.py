...
async def get_problem_handler(user_name: str, db: Session = Depends(deps.get_db)) -> List[ProblemResponseForDashboard]:
    problems = crud.get_all_problems(db)
    print("getProblemList called for user:", user_name)
    return [ProblemResponseForDashboard(problem=schemas.ProblemResponse(**problem.__dict__), status=problem.is_petit_coder) for problem in problems]
