...
def get_submission_by_submission_id(db: Session, submission_id: str) -> models.Submission | None:
    return db.query(models.Submission).filter(models.Submission.submission_id == submission_id).first()

def get_submission_by_user_name_and_id(db: Session, user_name: str, submission_id: str) -> models.Submission | None:
